...
class SpecialChannelStripComponent(ChannelStripComponent):
  ...
  def map_drum_notes(self, _oDevice, _hDevReg):
    self.alert('> DRUM: Bank %d, Strip %d' % (self.m_nCurrBank, self.m_nCurrStrip))
    nIdx = 0
    ...
    for oDrumPad in _oDevice.drum_pads:
      ...
      for oChainDev in oChain.devices:
        ...
        for oParam in oChainDev.parameters:
          ...
          self.log('> mapping volume to bank: %d, strip: %d, row: %d' % (nBank, nStrip, nRow))
